...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/*[local-name()='Relationships']">
		<xsl:copy>			
			...
			<xsl:apply-templates select="node() | @*"/>
			<!-- apply-templates pour les éléments <w:hyperlink> de endnotes.xml, dont on va tester l'@r:id  -->
			<xsl:apply-templates select="document(concat($tempdirSlash, '/word/endnotes.xml'))//w:hyperlink/@r:id"/>
		</xsl:copy>
	</xsl:template>

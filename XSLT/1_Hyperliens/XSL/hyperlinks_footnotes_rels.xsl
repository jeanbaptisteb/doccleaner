...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<!--
	Le paramètre $tempdir me passe le chemin absolu vers le répertoire temporaire du docx
	La template tempfolder ne contient que le nom du répertoire temporaire
	
	<xsl:variable name="tempfolder">
		<xsl:call-template name="substring_after_last">
			<xsl:with-param name="tempdir" select="$tempdir"/>
		</xsl:call-template>
	</xsl:variable>
	<xsl:template name="substring_after_last">
		<xsl:param name="tempdir"/>
		<xsl:param name="slash" select="'\'"/>
		<xsl:choose>
			<xsl:when test="contains($tempdir, $slash)">
				<xsl:call-template name="substring_after_last">
					<xsl:with-param name="tempdir" select="substring-after($tempdir, $slash)"/>
	...
	<xsl:template match="/*[local-name()='Relationships']">
		<xsl:copy>			
			...
			<xsl:apply-templates select="node() | @*"/>
			<!-- apply-templates pour les éléments <w:hyperlink> de footnotes.xml, dont on va tester l'@r:id  -->
			<xsl:apply-templates select="document(concat($tempdirSlash, '/word/footnotes.xml'))//w:hyperlink/@r:id"/>
		</xsl:copy>
	</xsl:template>
